...
class addAtlasNamestoCSV:
    def __init__(self,BNAtlasObj,HOAtlasObj,JuliechAtlasObj): # TODO Brainnetome = False (Default param)
                                                              # Keep all the other atlas objects as list objects
    ...
    def addNameCSV(self, csvPath, filename, column_index_include):
        ...
        for ix, row in enumerate(tqdm(dfMatrix)):
            ...
            # TODO Use 'if' condition to read the Hemispheres from the csv
            ...
            if not pd.isna(underConnectedMNI):
                ...
                # TODO construct the dictionary in the loop and then make it a dataframe
